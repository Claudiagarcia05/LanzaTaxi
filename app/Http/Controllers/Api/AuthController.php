...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($user->role === 'taxista') {
            ...
            if (Schema::hasColumn('taxistas', 'user_id')) {
                $taxista = Taxista::where('user_id', $user->id)->first();
            } elseif (Schema::hasColumn('taxistas', 'usuario_id')) {
                $taxista = Taxista::where('usuario_id', $user->id)->first();
            }

            if ($taxista) {
                $response['taxista'] = [
                    'id' => $taxista->id,
                    'licencia' => $taxista->licencia ?? $taxista->licencia_nro ?? null,
                    'matricula' => $taxista->matricula ?? null,
